...
def write_csvs_stellio(file_datetime, file_name):
    ...
    cpu_ram_logfile = "twins_to_compare/scripts_for_measures/stellio/measures/" + file_datetime + "-cpu_ram"
    mosquitto_logfile = "twins_to_compare/scripts_for_measures/stellio/measures/" + file_datetime + "-mosquitto"
    things_logfile = "twins_to_compare/scripts_for_measures/stellio/measures/" + file_datetime + "-entities"
    output_csv_things = f"twins_to_compare/scripts_for_measures/stellio/measures_csv/{file_name}-entities.csv"
    output_csv_mosquitto = f"twins_to_compare/scripts_for_measures/stellio/measures_csv/{file_name}-mosquitto.csv"
    result_file = "twins_to_compare/scripts_for_measures/stellio/results/" + file_name + "-delays.csv"

    os.makedirs("twins_to_compare/scripts_for_measures/stellio/measures_csv", exist_ok=True)
    ...
    with open(things_logfile, "r") as f, open(output_csv_things, "w", newline='') as csvfile:
        ...
        for line in f:
            print(line)
            print("with apiKey apikey123 successfully updated" in line)
            if "with apiKey apikey123 successfully updated" in line:
                timestamp = line[5:28].replace("T", "_").replace(":", "-")
                ts = datetime.strptime(timestamp, "%Y-%m-%d_%H-%M-%S.%f") + timedelta(hours=2)
                timestamp = ts.strftime("%Y-%m-%d_%H-%M-%S.%f")
